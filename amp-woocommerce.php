...
// 		5. Add Style's for Amp WooCommerce Page's
// 	*/
...
//    	// 2. Including Required Files
	...
	include_once(plugin_dir_path(__FILE__). '/inc/amp-woo-ajaxcalls.php');
	include_once(plugin_dir_path(__FILE__). '/inc/amp_woocommerce_features.php');
	include_once(plugin_dir_path(__FILE__). '/templates/layouts/product-review.php');

//    // 3.
    add_action('pre_amp_render_post','ampwoo_add_tree_shaking',10);

//    // 4. Overriding Ampforwp Plugin Single Files.
    	add_filter( 'amp_post_template_file', 'amp_woo_custom_woocommerce_template', 11, 3 );

//    // 5.
     add_action('wp','ampwoo_main_css',10);
   ...
   // 3. Add Custom Style for WooCommerce Page.
	function ampwoo_add_tree_shaking (){
    global $redux_builder_amp;
	add_filter('wc_get_template','ampwoo_woocommerce_template_override',10,5);
	add_filter('ampforwp_body_class','ampwoo_add_body_classes',10,2);
	add_filter('amp_content_sanitizers','ampwoo_add_woocommerce_blacklist',100);
