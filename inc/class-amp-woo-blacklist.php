...
if ( class_exists( 'AMP_Blacklist_Sanitizer' ) ) {
	class AMP_WOO_Blacklist extends AMP_Blacklist_Sanitizer {
		...
		private function strip_attributes_recursive( $node, $bad_attributes, $bad_protocols ) {
			...
			if($node->nodeName=='a' && $node->hasAttribute('href') && function_exists('ampforwp_woocommerce_findInternalUrl') ){
				$href = $node->getAttribute('href');
				if((function_exists('is_woocommerce') && is_woocommerce()) || (function_exists('is_checkout') && is_checkout()) || (function_exists('is_cart') && is_cart()) || (function_exists('is_account_page') && is_account_page())){
				$node->setAttribute('href', \ampforwp_woocommerce_findInternalUrl($href));
			    }else{
